...
%%
%% Copyright (c) 2011-2016 Kristoffer Rose <krisrose@crsx.org>
...
\makeatother
%%
%% Pick one of the following canned styles:
\renewcommand{\rmdefault}{pplx}\usepackage{eulervm}\AtBeginDocument{\SelectTips{eu}{11}}
%\usepackage{ts}\AtBeginDocument{\SelectTips{lu}{11}}
...
$ ^textcolorinput{^texttt{rm -fr ~/.hacs hacs}}
  \end{code}
  %%$
  Now retrieve the \emph{hacs-\version.zip} archive, extract it to a new directory, and install it, for
  example with the following commands:\footnote{User input is \textcolorinput{like this}.}
  %%
  \begin{code}[commandchars=\^\{\}]
...
$ ^textcolorinput{^texttt{./First.run --scheme=Compile \}}
  ...
  %%
  \begin{code}[commandchars=\^\{\}]
energon1[hacs]$ ^textcolorinput{^texttt{alias hacs=$HOME/.hacs/bin/hacs}}
  \end{code}
  %%
...
\begin{example}[comments]
  ...
  character class means ``everything but'').
...
\begin{commands}[lexical analysis]
  The fragment in Example~\ref{ex:lexical} is part of \emph{First.run} from Section~\ref{sec:intro},
  which can be used just as a lexical analyzer.  This is achieved by passing two arguments to the
  \emph{First.run} command: a \emph{token sort} and a \emph{token term}.\footnote{The command has
    more options that will be introduced as needed.}  Execution proceeds by parsing the string
  following the syntax of the token. For example, the following checks the lexical analysis of a
  number:
  \begin{code}[commandchars=\\\{\}]
...
2 + 3 * ( 4 + 5 )
  ...
  %%
  \begin{code}[commandchars=\\\{\}]
$ \textcolorinput{./First.run --sort=Exp --term="2 ** 3"}
Exception in thread "main" java.lang.RuntimeException: net.sf.crsx.CRSException: 
...
Leftmost(⟦⟨FLOAT#⟩⟧)                  →   ⟦⟨FLOAT#⟩⟧ ;
  ...
  they have the wrong sort. Instead it uses syntax construction to obtain the correct sort by making
...
\begin{commands}[invoke scheme]
  ...
  %%
  \begin{code}[commandchars=\\\{\}]
$ \textcolorinput{./First.run --scheme=Leftmost --sort=Exp --term="2*3+4"}
 2
  \end{code}
  %%$
...
HACS \version
…
$ \textcolorinput{./Stack.run --scheme=Compile --term="(1+2)*(3+4)"}
  PUSH 1 
  PUSH 2 
  ADD 
  PUSH 3 
...
\begin{example}
  ...
  example like this:
  \begin{code}[commandchars=\\\{\}]
$ \textcolorinput{hacs SZ.hx}
…
$ \textcolorinput{./SZ.run --scheme=Load --term="s 0+s 0"}
...
$ \textcolorinput{hacs SZ.hx}
…
$ \textcolorinput{./SZ.run --scheme=Calc --term="s 0+s 0"}
 s  s  0   
  \end{code}
...
…
$ \textcolorinput{./Bool.run --scheme=Eval --term='t|(¬f)'}
 t
  \end{code}
\end{example}
...
\begin{example}
  ...
  %% 
  Rule \thetag{3} uses the environment to attach a synthesized attribute $t$ onto an $E$ that
  ...
  %%
  Finally, rule \thetag{4} is included to exemplify what to do with rules that do not propagate the
  environment.
  ...
  \item Next, encode the simplest rule, \thetag{2}. As in the previous section, observe that
   ...
   Ee(⟦⟨E#1⟩ + ⟨E#2⟩⟧)↓e{:#Ee)
    ...
    expression and an "e" attribute. Since there are no complicated dependencies in \thetag{2}, the
   ...
   Ee(⟦⟨E#1⟩ + ⟨E#2⟩⟧)↓e{:#Ee)  →  ⟦⟨E Ee(#1)↓e{:#Ee)⟩ + ⟨E Ee(#2)↓e{:#Ee)⟩⟧ ;
    ...
    equations $E_1.e=E.e$ and $E_2.e=E.e$ from \thetag{2}.
  ...
  \item Next, consider rule \thetag{3}, which defines the synthesized attribute $t$ on
    \textbf{id}-expressions using a ``Lookup'' meta-function that is meant to suggest extracting the
    mapping from the $E.e$ environment inherited from the context. Begin with a template like this:
    \begin{hacs}
   Ee(⟦⟨ID#1⟩⟧)  →  ⟦⟨ID#1⟩⟧ ;
    \end{hacs}
    which just states that the $e$ attribute is inherited ``into'' a name subterm (through "Ee"),
    essentially making $E.e$ available, but note that the rule does not use the inherited
    attribute. In particular, this rule fails to actually set the $t$ attribute. Fixing this entails
    first capturing the ``Lookup'' in the inherited environment. \HAX has a special notation for
    this. Write the pattern as follows:
    \begin{hacs}
   Ee(⟦⟨ID#1⟩⟧)↓e{#1 : #t}  →     …
    \end{hacs}
    The pattern of this rule is equipped with a \emph{mapping constraint} on the "e" inherited
    attribute, which corresponds to the "Lookup" notation of \thetag{3} above. It will match when
    "Ee" is applied to a name, called "#1", which is \emph{also} mapped by the associated "e"
    attribute to a type, denoted~"#t". After capturing the mapped type this way, complete the rule
    by explicitly associating it to the "t" attribute associated with "ID" expressions.
    \begin{hacs}
   Ee(⟦⟨ID#1⟩⟧)↓e{#1 : #t}  →   ⟦⟨ID#1⟩⟧ ↑t(#t) ;
    \end{hacs}
    Notice how mappings like "e" use "{}" notation for both declarations and constraints, and simply
    valued attributes like "t" use~"()". Also note that we did not include any "↓e{:#Ee}"
    fragments, because the attribute is not passed anywhere.

  \item The semantic rules of the SDD can now be completed by encoding \thetag{1}. Begin with
    \begin{hacs}
   Se(⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E#3⟩; ⟨S#4⟩⟧)↓e{:#Se}
     →  ⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E Ee(#3)↓e{:#Se}⟩; ⟨S Se(#4)↓e{:#Se}⟩⟧ ;
    \end{hacs}
    ...
    \begin{hacs}
   Se(⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E#3⟩; ⟨S#4⟩⟧)↓e{:#Se}
     →  ⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E Ee(#3)↓e{:#Se}⟩; ⟨S Se(#4)↓e{:#Se}↓e{#2 : #1}⟩⟧ ;
    \end{hacs}
  ...
  \item Finally, \thetag{4} does not contain any semantic rules but a case must still be provided
    for the scheme:
    \begin{hacs}
   Ee(⟦⟨NUM#1⟩⟧)  →  ⟦⟨NUM#1⟩⟧ ;
    \end{hacs}
  ...
  follows.
  \begin{hacs}
 Se(⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E#3⟩; ⟨S#4⟩⟧ ↑#syn)↓e{:#Se}
   →   ⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E Ee(#3)↓e{:#Se}⟩; ⟨S Se(#4)↓e{:#Se}↓e{#2 : #1}⟩⟧ ↑#syn ;

 Ee(⟦⟨E#1⟩ + ⟨E#2⟩⟧ ↑#syn)↓e{:#Ee}  →  ⟦⟨E Ee(#1)↓e{:#Ee}⟩ + ⟨E Ee(#2)↓e{:#Ee}⟩⟧ ↑#syn ;
 Ee(⟦⟨ID#1⟩⟧ ↑#syn) ↓e{#1 : #t}  →   ⟦⟨ID#1⟩⟧ ↑#syn ↑t(#t) ;
 Ee(⟦⟨NUM#1⟩⟧ ↑#syn)  →  ⟦⟨NUM#1⟩⟧ ↑#syn ;
  \end{hacs}
\end{example}
...
%%TODO: Example with shared carrier.
...
\begin{example}
  ...
  \begin{code}[commandchars=\^\{\}]
$ ^textcolorinput{^texttt{hacs LetrecMap.hx}}
…
$ ^textcolorinput{^texttt{./LetrecMap.run --scheme=Reduce --term="a:b b:c in a"}}
c
$ ^textcolorinput{^texttt{./LetrecMap.run --scheme=Reduce --term="b:c a:b in a"}}
c
  \end{code}
\end{example}
...
Ee(⟦⟨Exp#1⟩ + ⟨Exp#2⟩⟧↑#syn)↓e{:#Ee} → ⟦ ⟨Exp Ee(#1)↓e{:#Ee}⟩ + ⟨Exp Ee(#2)↓e{:#Ee}⟩ ⟧↑#syn ;
Ee(⟦⟨Exp#1⟩ * ⟨Exp#2⟩⟧↑#syn)↓e{:#Ee} → ⟦ ⟨Exp Ee(#1)↓e{:#Ee}⟩ * ⟨Exp Ee(#2)↓e{:#Ee}⟩ ⟧↑#syn ;
Ee(⟦⟨INT#⟩⟧↑#syn) → ⟦⟨INT#⟩⟧↑#syn ;
...
Se(⟦⟨ID#v⟩ := ⟨Exp#1⟩; ⟨Stat#2⟩⟧↑#syn) ↓e{:#Se}
  → SeB(⟦⟨ID#v⟩ := ⟨Exp Ee(#1)↓e{:#Se}⟩; ⟨Stat#2⟩⟧↑#syn) ↓e{:#Se};
{
  | scheme SeB(Stat) ↓e;  // helper scheme for assignment after expression type analysis
  SeB(⟦⟨ID#v⟩ := ⟨Exp#1↑t(#t1)⟩; ⟨Stat#2⟩ ⟧↑#syn) ↓e{:#Se}
   → ⟦⟨ID#v⟩ := ⟨Exp#1⟩; ⟨Stat Se(#2)↓e{:#Se}↓e{#v : #t1}⟩⟧↑#syn ;
}

Se (⟦ { ⟨Stat#1⟩ } ⟨Stat#2⟩ ⟧↑#syn) ↓e{:#Se}
 → ⟦ { ⟨Stat Se(#1)↓e{:#Se}⟩ } ⟨Stat Se(#2)↓e{:#Se}⟩ ⟧↑#syn ;
...
\begin{example}[untyped λ calculus]
  ...
  %%
  \begin{code}[commandchars=\\\{\}]
$ \textcolorinput{hacs Lambda.hx}
…
$ \textcolorinput{./Lambda.run --sort=T --term='(λx.x)y'}
 y
  \end{code}
  %%
\end{example}
...
Append(⟦⟧, #S2) → #S2 ;
Append(⟦ old := ⟨Exp#2⟩; ⟨Stat#3[Exp⟦old⟧]⟩ ⟧, #S2)
  → ⟦ new := ⟨Exp#2⟩; ⟨Stat Append(#3[Exp⟦new⟧], #S2⟩ ⟧ ;
  \end{hacs}
  %%
  ...
  it as "#3[Exp⟦old⟧]". This setup makes it possible to now \emph{systematically replace the symbol}
  with a new and fresh one, referred to as "new" in the replacement in line 16, but which the system
  will in fact replace with a new and unique name, both the binder \emph{and all the in-scope
    occurrences} inside~"#3". This ensures that there is no so-called ``variable capture,'' \ie,
  that occurrences of the "old" name inside "#S2" accidentally become double-bound.
  %%
\end{example}
...
syntax error possible with HOAS without explicitly substituting the occurrences of the bound
variable with something else in the copy, which would expose the error. (You may think that this is
an obvious mistake that no one would make, but this sort of variable escaping is a real problem that
...
\begin{figure}[t]
  \begin{figureunit}[
      \caption{Using \HAX abstractions to create templates (\emph{examples/Template.hx}).}
      \label{fig:template}
    ]
    \inputhacs[texcl,mathescape,xleftmargin=\parindent,xrightmargin=2em,numbers=right]{../samples/Template.hx}
  \end{figureunit}
\end{figure}

\begin{figure}[t!]\scriptsize
  \begin{figureunit}[
      \caption{Managing labels and names.}
      \label{fig:jose}
    ]
    \inputhacs[xleftmargin=\parindent,xrightmargin=2em,numbers=right]{../samples/JoseMaps.hx}
  \end{figureunit}
\end{figure}

\begin{example}[templates]
  Figure~\ref{fig:template} shows how one can use a higher order ``raw'' abstraction to define a
  template that can be used for several cases. The "Flip" scheme will just call the "Flip2" scheme,
  in each rule (in lines 9--10) packing up what syntax it actually uses in an abstraction:
  %%
  "[s1,s2] ⟦⟨Sum s1⟩+⟨Sum s2⟩⟧" should be read ``an abstraction that when \emph{applied} to two
  arguments, which we call "s1" and "s2", generates the syntactic construct
  %%
  "⟦⟨Sum s1⟩+⟨Sum s2⟩⟧" with "s1" and "s2" replaced by the actual arguments.''  If needed, the
  term after the "[]"s can be a semantic term instead, it just needs to use the variables where the
  arguments should be replaced.  As can be seen in line 14, the syntax for application is in a sense
  already ``raw,'' and thus does not change.

  While the ``flipping'' example shown here does not really save lines, the technique is very useful
  in large specifications with groups of symbols with similar behaviour. Here is what a run looks
  like:
  %%
  \begin{code}[commandchars=\\\{\}]
$ \textcolorinput{hacs Template.hx}
…
$ \textcolorinput{./Template.run --scheme=Flip --term="a+b+c"}
 (  c  +  b  ) +  a
  \end{code}
\end{example}

\begin{example}
  Figure~\ref{fig:jose} is a more realistic example showing how to use an symbol sort and a normal
  identifier by exploiting HACS built-in maps to store the latter and lookup with the former. The
  specification uses ``identifiers'' for the source grammar, lines 7--8, however, the target
  language uses automatic symbols for its special "Label" sort, line~16. Line 28 declares our
  mapping attribute. Lines 31--32 is a helper to add four to an integer string; such rules are
  explained in the next section.

  The new stuff is the "CompileS" scheme in lines 34--40. Line 35 is the pattern for a variable
  declaration in the source grammar. It rewrites to a target contraction in line 36, which has two
  special features: it creates a ``fresh'' new "Label", "id", and it records that label in the "loc"
  environment to pass on this information to the next statement and move the current offset as
  appropriate. The rule also adds a quick comment with the original "ID" for readability.
  
  The following rule in line 37--38 replaces \emph{uses} of a source variable by lookup of label
  using our constant "ID". (This ignores the case where "#1" is not in the map for simplicity.) The
  rest of the "CompileS" rules are normal, and the rest of the script has standard helpers.
\end{example}

Finally we show an example of using higher order syntax with semantic structures, where the syntax
for abstractions is different.
...
\begin{example}
  Finally consider a real example from the literature~\cite{DanvyRose:rta1998}. Figure~\ref{fig:cps}
  shows a system for transforming λ-terms to continuation-passing style with call-by-value
  evaluation semantics. Lines 14--19 specify a \emph{one-pass} variant of continuation-style
  conversion. Notice how the transformation defines a syntactic helper scheme to make the recursive
  composition easier to express.

  The script compiles and generates code as follows:
  \begin{code}[commandchars=\\\{\}]
$ \textcolorinput{hacs CPS.hx}
…
$ \textcolorinput{./CPS.run --scheme=CPS --term="(λx.x x)"}
...
\begin{table}[t]
  \begin{figureunit}[
    ...
    \begin{displaymath}
      \begin{array}{c|p{.7\linewidth}}\multicolumn{2}{c}{}\\
        \Xhline{2\arrayrulewidth}
        \textit{Expression} & \textit{Explanation} \\
        \Xhline{2\arrayrulewidth}
        \#x & refer to Computed value or token string $\#x$ \\
        \texttt{\$}\,\#x & extract integer value from token string $\#x$ \\
        \texttt{\%}\,\#x & convert integer Computed value $\#x$ to string value \\
        \verb|"foo"|\quad\texttt{42}\quad\texttt{0x2A} & literal strings and integers \\
        \texttt{(}E\texttt{)} & grouping \\
        \hline
        E\,\texttt{[}E\texttt{:}E\texttt{]} & substring (optional indices are
          \texttt{[}\emph{first}\texttt{:}\emph{after-last}\texttt{]} with $0$-based indexing) \\
        \hline
        \texttt{+}E\quad\texttt{-}E & unary integer plus and minus \\
        \verb|~|E & bitwise not \\
        \texttt{length}\,E & length of (Unicode) string \\
        \texttt{up-case}\,E & convert string to upper case \\
        \texttt{down-case}\,E & convert string to lower case \\
        \texttt{escape}\,E & convert all special characters to their Java escape equivalent \\
        \texttt{unescape}\,E & convert all Java escape sequences to special characters \\
        \texttt{trim}\,E & remove leading and trailing spacing \\
        T\,E & generate constants in user's type $T$ \\
        \hline
        E\texttt{*}E\quad E\texttt{/}E\quad E\texttt{\%}E & integer multiplication, division, modulo \\
        E\texttt{\&}E & bitwise and \\
...
...
$ \textcolorinput{./MakeToken.run --scheme=Test --term="Hello Hello Hello John"}
X_Hello_x_x_John
  \end{code}
  %%$
\end{example}
...
\begin{example}
  Figure~\ref{fig:desk} implements a simple desk calculator. Notice how it uses the "$"
  marker to import a token as an integer value of sort "Computed". %$
  %%
  Here is a run:
  %%
\begin{code}[commandchars=\\\{\}]
$ \textcolorinput{hacs Desk.hx}
...
$ \textcolorinput{./Desk.run --scheme=Eval --term="1+2*3/(2-1)"}
7.0
  \end{code}
  %%$
\end{example}
...
\begin{example}[finding cats]
  ...
  %%
\begin{code}[commandchars=\\\{\}]
$ \textcolorinput{hacs IsCat.hx}
$ \textcolorinput{./IsCat.run --scheme=IsCat --term="dog"}
False
$ \textcolorinput{./IsCat.run --scheme=IsCat --term="cat"}
True
  \end{code}
  %%$
\end{example}
...
\begin{example}
  ...
  \begin{code}[commandchars=\\\{\}]
$ \textcolorinput{hacs Symbols.hx}
$ \textcolorinput{./Symbols.run --scheme=Test --term="A A a a * A * a"}
...
\begin{example}[set of words]
  ...
  %%
\begin{code}[commandchars=\^\{\}]
$ ^textcolorinput{^texttt{hacs WordSet.hx}}
$ ^textcolorinput{^texttt{./WordSet.run --scheme=Check --term="a in a,b,b,a"}}
Yes, the list has a.
$ ^textcolorinput{^texttt{./WordSet.run --scheme=Check --term="Foo in Bar"}}
...
\begin{code}[commandchars=\^\{\}]
$ ^textcolorinput{^texttt{hacs WordMap.hx}}
$ ^textcolorinput{^texttt{./WordMap.run --scheme=Substitute --term="a:b in a b b a"}}
b b b b
$ ^textcolorinput{^texttt{./WordMap.run --scheme=Substitute --term="k:v in a b c"}}
a b c
\end{code}
%%$
\end{example}
...
\begin{figure}[p]
  \begin{figureunit}[
    ...
    \inputhacs[xleftmargin=\parindent,xrightmargin=2em,numbers=right,texcl]{../samples/WordSubst.hx}
  \end{figureunit}
\end{figure}
...
$ \textcolorinput{./WordSubst.run --scheme=Run --term="a=1 a"}
 a=1  1   
$ \textcolorinput{./WordSubst.run --scheme=Run --term="b a \{a=1 b=2\}"}
 2  1   {  a=1  b=2    }     
$ \textcolorinput{./WordSubst.run --scheme=Run --term="\{a=1 b=2 c=3\} a b c \{a=4\} a b c"}
