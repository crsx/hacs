...
sort Name  | symbol ⟦⟨ID⟩⟧ ;
  ...
  (we discuss this at length in Section~\ref{sec:hoas}).
  %%
\end{example}
...
\begin{example}
  ...
  semantic "Value" form. When we try to run the script on a single example:
  \begin{code}[commandchars=\\\{\}]
$ \textcolor{blue}{hacs SZ.hx}
…
$ \textcolor{blue}{./SZ.run --scheme=Trans --term="s0+s0"}
« $Print2-SZ$Value[SZ$Value_Succ[SZ$Value_Succ[SZ$Value_Zero]], 0] »
  ...
  we get the right result, which is the Peano numeral for $2$, however, \emph{it is not printed
    ...
    structures have no syntax so cannot be printed}.
\end{example}
...
\begin{notation}[attributes]\leavevmode
  ...
  \item Attributes are defined with attribute declarations:
    ...
    \item The last unit gives the category and sort(s) of the attribute: $(S)$ is a simply valued
      ...
      either a token sort or a sort with a single "symbol" case.
    \end{itemize}
...
\item If a rule is prefixed with the special syntax "[global x]", then the symbol "x" will not be
  subject to this automatic conversion.\footnote{Note that for now the "x" cannot be quoted so you
    must use the raw form.}
