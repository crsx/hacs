...
%% {\tracingall\makeatletter
%%  \let\e@=\expandafter 
%%  %\e@\long\e@\gdef\e@\getversion\e@{\e@\setversion\@@input ../VERSION !!!}%
%%  \long\def\settoksz@#1!!!{\toks0={#1}}%
%%  \e@\settoksz@ \@@input ../VERSION !!!
%%  \def\setversion VERSION=#1!!!{\def\version{#1}}%
%%  %\getversion
%%  \e@\setversion\thetoks0!!!
%% }
%% %\setversion VERSION=1.0.1@
\def\version{1.0.7\xspace}
...
%%% TODO: blurb on symbol tables/higher order syntax.
...
concrete syntax details managed through precedence annotations and ``syntactic sugar'' declarations.
...
\begin{example}[scheme over syntax]\label{ex:leftmost}
  ...
  define a new scheme, "Leftmost", which for an expression returns the leftmost number of the
  expression. We achieve this by first declaring that ASTs of the sort can be generated by the
  "scheme", as follows:
  %%
...
sort Exp | scheme Leftmost(Exp) ;
  \end{hacs}
  %%
  The declaration states that the scheme takes one parameter, of "Exp" sort in ()s, and delivers a
  result that is also in sort "Exp". (Later we shall see translations that convert from one sort to
  another.)
...
The procedure for defining a syntax-directed schema is:
%%
\begin{itemize}

\item Set the result "sort" and add a declaration for the scheme. A scheme "S" of result sort "R"
  with argument sorts "A" and "B" has the shape.
...
written with semantic constructors.

The final two examples illustrate this: one defines a ``syntactic scheme,'' the other ``semantic
data.''

Syntactic schemes are schemes that are defined using "⟦…⟧" nootation. They are very similar to
...
\begin{example}[semantic data]\label{ex:unif}
  %%
...
into a \HAX one, introducing the necessary \HAX formalisms as we go.
...
\item The second thing to do is to create patterns from the abstract syntax production, just like in
  the previous section: the pattern for the single production is
  \begin{hacs}
   ⟦ ⟨E#1⟩ + ⟨E#2⟩ ⟧
  \end{hacs}
...
\end{enumerate}
%%
If we assume that "Unif" refers to the semantic scheme defined in Example~\ref{ex:unif}, then we are
done.
...
``bottom-up'' synthesis of named attributes. The last component used in traditional compiler
specification is the use of \emph{inherited} named attributes, which are distributed top-down, like
parameters. \HAX supports this with a hybrid combination of implicit named parameters for recursive
schemes, that we describe here.

Furthermore, one of the main uses of inherited attributes in formal compiler specifications is
\emph{symbol management}. We shall focus here on the \HAX notion of \emph{environment}, which fills
this niche and cannot be easily achieved with usual positional arguments to recursive schemes.
...
\begin{hacs}
   Se(⟦⟨T#1⟩ ⟨Name#2⟩ = ⟨E#3⟩; ⟨S#4⟩⟧)  →      ⟦⟨T#1⟩ ⟨Name#2⟩ = ⟨E Ee(#3)⟩; ⟨S Se(#4)⟩⟧ ;
\end{hacs}
  ...
  map.
\begin{hacs}
   Se(⟦⟨T#1⟩ ⟨Name#2⟩ = ⟨E#3⟩; ⟨S#4⟩⟧)
   →   ⟦⟨T#1⟩ ⟨Name#2⟩ = ⟨E Ee(#3)⟩; ⟨S Se(#4) ↓e{#2 : #1}⟩⟧ ;
\end{hacs}
...
\end{figure}
%%
To summarize, with $D$ denoting data terms and $F$ ``function'' terms, \ie, applied schemes, we have
the conventions of Figure~\ref{fig:attributes} in \HAX.  Note that one can have several constraints
on a single (data or function) term, as long as they are all properly declared (for the appropriate
sort or scheme, respectively).
...
Append(⟦ old := ⟨Exp#2⟩; ⟨Stat#3[old]⟩ ⟧, #S2)
  ...
  pattern matching to also keep track of all the occurrences inside the scope, which is~"#3". WIth
  this setup, we can now \emph{systematically replace the symbol} with a new and fresh one, which we
  call "new" in the replacement in line 16, but which the system will in fact replace with a new and
  unique name, both the binder \emph{and all the in-scope occurrences} inside~"#3". In this way, we
  ensure that there is no so-called ``variable capture,'' \ie, that occurrences of the "old" name
  inside "#S2" accidentally get double-bound.
...
\end{hacs}
but this is not immediately possible with HOAS without explicitly substituting the occurrences of
the bound variable with something else, which would expose the error. (You may think that this is an
obvious mistake that noone would make but variable escaping is a real problem that leads to bugs in
compilers.)

The use of HOAS in \HAX in fact allows the full range of methods used in higher-order
rewriting~\cite{Jouannaud:klop2005}, and is a very powerful mechanism.
...
\item Meta-variables denoting tokens prefixed with "$". %$
\end{itemize}
%%
Notice that meta-variables are part of the syntax: essentially "⟨Computed#x⟩" and "#x" mean the
same.  The allowed operators are summarized in Figure~\ref{fig:ops}.
...
\begin{example}[count]\label{ex:count}
  Consider the list from Example~\ref{ex:append}. The following computes the length of the list, using a helper.
  %%
\begin{hacs}
...
in Figure~\ref{fig:desk}, which also shows the use of "$". %$
