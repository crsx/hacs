...
compilers and other systems for manipulating code. As the name suggests, the notation combines HOAS
with concepts from attribute grammars~\cite{Knuth:mst1968} and other common compiler specification
...
\end{hacs}
%% 
We assume that the "T" sort is defined elsewhere with the possible types.

\item Once the grammar is defined, we can declare the new attribute:
  \begin{hacs}
   attribute ↓e{ID : T} ;
\end{hacs}
Like synthesized attributes, inherited attributes are always given lowercase names.
...
\begin{example}[untyped λ calculus]
  ...
  \item The third production (line 8) is also special: here we declare that if a term is a simple
    "ID", then in fact it is a \emph{symbol}, which is required for identifiers to occur bound as
    sort~"T".

  \item Finally, we use the defined scoping in the classic $β$ rewrite rule for our one scheme in
    line~12: it expresses that a λ term which is an application (our scheme declared in line~7)
    where the first subterm is a λ-abstraction (declared in line~6) can be rewritten. The rewriting
...
\item In grammar productions, for a token $T$ and a \HAX variable $x$, we can use the special
...
Also remember that \emph{all} computed parts of a rule are evaluated as soon as the rule is
expanded, so only use this for calculuations that can be safely computed in any case where the
\emph{pattern} of the rule matches.
...
\begin{manual}[parsed terms]\label{man:parsed}
  ...
  \item The special term "error⟦…⟧" will print the error message embedded in "⟦…⟧", where one is
    ...
    evaluated \emph{when the rule is expanded}, thus only use "error" when the \emph{pattern} of the
    rule guarantees that you want the error.
