...
\begin{example}
  ...
  \item Next, encode the simplest rule, \thetag{2}. As in the previous section, observe that
   ...
   Ee(⟦⟨E#1⟩ + ⟨E#2⟩⟧)
    ...
    "Ee" scheme recursively to the subexpressions that should inherit the "e" attribute, which are
    the two marked subexpression, \emph{inside the syntax markers}:
    \begin{hacs}
  ...
  \item The semantic rules of the SDD can now be completed by encoding \thetag{1}. Begin with
   ...
   →   ⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E Ee(#3)⟩; ⟨S Se(#4) ↓e{#2 : #1}⟩⟧ ;
    ...
    second wraps raw (non-syntax) fragments inside syntax. Attribute constraints are always in the
    raw fragments.
  ...
  follows.
  \begin{hacs}
 Se(⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E#3⟩; ⟨S#4⟩⟧ ↑#syn)
   →   ⟦⟨T#1⟩ ⟨ID#2⟩ = ⟨E Ee(#3)⟩; ⟨S Se(#4) ↓e{#2 : #1}⟩⟧ ↑#syn ;
