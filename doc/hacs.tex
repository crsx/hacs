...
\end{hacs}
  ...
  \item "token" declarations in particular have the "token" keyword followed by a regular expression
    between a "|" (vertical bar) and a ";" (semicolon). It defines the token as a \emph{terminal
      symbol} that can be used in other token declarations as well as the syntax productions
...
In compiler construction formalization we can express this using an SDD with an inherited attribute
that for each node in the AST associates the appropriate symbol table for that node. Consider the
...
\begin{equation*}
  \begin{array}{l|ll}
    ...
    &
    \textbf{name}_1.type = \op{Lookup}(E.e, \textbf{name}_1.sym)
    &\thetag{3}
    \\[\jot]
    \hline
  \end{array}
...
Finally, rule \thetag{3} uses the environment to attach a (synthesized) attribute $type$ to each
\textbf{name} token. Specifically, the notation is meant to suggest that the type value is obtanied
by a \emph{lookup} of the mapping for the symbol of $\textbf{name}_1$.
...
\end{hacs}
  ...
  Note that only sorts with a "symbol" declaration can be used for keys of mappings: as suggested
  above, the mappings take the rôle of \emph{symbol tables} from traditional compilers.
...
\item The second thing to do is to associate the inherited attribute to one or more \emph{recursive
  ...
  certain sort.  This has to be done separately for each sort that $e$ propagates over, so we invent
  a scheme for each of $S$ and $E$:
\begin{hacs}
   sort S | scheme Se(S) ↓e ;
   sort E | scheme Ee(E) ↓e ;
\end{hacs}
  As can be seen, the scheme generates results of the "S" and "E" sorts and also takes a single
  parameter of the "S" and "E" sorts, respectively, which each \emph{carry} the inherited
  attribute~"e".

\item \textbf{More Steps -- This Section Not Complete}
