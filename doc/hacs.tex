...
%% Topmatter.
\title{
  Compiler Generation Using \HAX\thanks{This version describes \HAX β version 0.9.1 released for use at NYU.}
}
\author{
  Kristoffer H. Rose\\
...
\begin{commands}[install \HAX]\label{com:all}
  Retrieve the \emph{hacs-0.9.1.zip} archive, extract it to a new directory, and install it, for
  example with the following commands:\footnote{User input is \textcolor{blue}{blue}.}
  %%
\begin{code}[commandchars=\^\{\}]
energon1[~]$ ^textcolor{blue}{^texttt{wget http://crsx.org/hacs-0.9.1.zip}}
energon1[~]$ ^textcolor{blue}{^texttt{unzip hacs-0.9.1.zip}}
