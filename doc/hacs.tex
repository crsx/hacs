...
%% Topmatter.
\def\version{0.9.5\xspace}
\title{
  Compiler Generation Using \HAX\thanks{This version describes \HAX β version \version released for use at NYU.}
}
\author{
  Kristoffer H. Rose\\
...
\end{hacs}
  ...
  \emph{parameters} in the form of ``meta-variables'' starting with "#" (hash), like "#1", to
  designate ``function arguments'' that should be copied from the pattern to the replacement.
...
energon1[~]$ ^textcolor{blue}{^texttt{./WordSet.run --scheme=Check --term="a in b"}}
No, the list does not contain a.
\end{code}
\end{example}
...
\begin{example}[map of words]
  Figure~\ref{fig:wordmap} shows how an environment can be synthesized and then used as an
  environment. The pattern is similar to the set example, except here we not only synthesize the map
  attribute "m" but also ``copy'' it over to an inherited map -- an environment -- "e". Notice these
  extras:
  %%
  \begin{itemize}
...
\begin{hacs}[numbers=right,texcl]
module org.crsx.hacs.samples.WordPairs {
...
CheckMapping(#key, MoreMappings(#key, #value, #map), #pam)
  → ⟦Yes, the map takes ⟨WORD#key⟩ to ⟨WORD#value⟩.⟧ ;

// Fall back to a default recursive case.
default CheckMapping(#key1, MoreMappings(#key2, #value, #map), #pam)
  → CheckMapping(#key1, #map, #pam) ;

// If the map and the first member of the list are the same, then we succeed!
CheckMapping(#key, NoMappings, MoreMappings(#key, #value, #pam))
  → ⟦No, but map gets ⟨WORD#key⟩ from ⟨WORD#value⟩.⟧ ;

// Fall back to a default recursive case.
default CheckMapping(#key1, NoMappings, MoreMappings(#key2, #value, #pam))
  → CheckMapping(#key1, NoMappings, #pam) ;
...
\end{hacs}
\caption{\emph{WordPairs.hx}: Word Mapping by Pairs.}
\label{fig:wordpairs}
\end{figure}

\begin{example}[using a list as an environment]
  We can also synthesize information more manually.  Figure~\ref{fig:wordpairs} shows a similar
  example, where we want to search \emph{pairs} of words.  However, unlike in the set and map
  examples above, we have not chosen to generate the map on the fly but instead generate it as a
  semantic data structure that we use traditional pattern matching to access.

  Notice the following:
  \begin{itemize}

  \item In line 38 we restrict the rule to cases where the first parameter "#key" is \emph{the same}
    as the first "#key" embedded in the "Mappings" data structure, by explicitly using the same
    meta-variable name in both places.

  \item Line 42 does not have the constraint: the two keys can have any values. Therefore the rules
    is marked "default" to make sure it is tried \emph{after} the rule in line 38. (In normal
    operation, rules are \emph{not} tried in any particular order.)
