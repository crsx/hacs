...
\end{hacs}
  ...
  \item Each non-terminal and token reference has the non-terminal name followed by a "#"$n$ marker
    to identify the subexpression of that non-terminal sort for use on the right side of the
    "→". So, for example, the grammar case
\begin{hacs}
   | ⟦ ⟨Exp@2⟩ * ⟨Exp@3⟩ ⟧@2
\end{hacs}
    leads to a \emph{syntactic pattern}
\begin{hacs}
   ⟦ ⟨Exp#1⟩ * ⟨Exp#2⟩ ⟧
\end{hacs}
    where we have removed the "@" precedence markers and introduced the ``disambiguation'' markers
    "#1" and "#2" so the rule can refer to the left one after the~"→".  We can then \emph{insert}
    the pattern in one case of the scheme, which gives us the beginning of the second rule
\begin{hacs}
   Leftmost(⟦⟨Exp#1⟩*⟨Exp#2⟩⟧)  →     …
\end{hacs}
    Notice how the core of the syntax specification -- "⟦⟨Exp⟩*⟨Exp⟩⟧" -- is \emph{all} present still.
