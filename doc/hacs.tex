...
energon1[~]$ ^textcolor{blue}{^texttt{rm -fr ~/.hacs}}
\end{code}
  %%
...
\begin{example}\label{ex:ast}
  Consider again the example expression grammar of Example~\ref{ex:syntax}:
  %%
\begin{hacs}[xleftmargin=\parindent]
sort Exp  | ⟦ ⟨Exp⟩ + ⟨Exp@1⟩ ⟧
          | ⟦ ⟨Exp@1⟩ * ⟨Exp@2⟩ ⟧@1
...
\end{hacs}
  %%
  This grammar serve two purposes: to describe all token sequences that can be parsed, and to
...
\begin{definition}
  A scheme is (abstract) \emph{syntax-directed} if it has one case per abstract syntax production.
\end{definition}
...
\begin{example}[scheme over syntax]\label{ex:leftmost}
  ...
  expression. We achieve this by first declaring that the sort has a "scheme", as follows:
  %%
...
\item Set the result "sort" and add a declaration for the scheme, starting with a "|" and "scheme",
  followed by the shape of the scheme, which is a capitalized name followed by the sorts of the
  arguments in ()s; for the scheme "S" of result sort "R" and argument sorts "A1" and "A2" this has
  the shape.
\begin{hacs}
  sort R |  scheme S(A1, A2);
\end{hacs}
...
\item To make the scheme "S" syntax-directed in, say, "A1", then create a separate rule for each
  ``pattern case'' of "A1", which is just each abstract syntax production for "A1" with a "#"$n$
...
\end{hacs}
  ...
  that in the first rule, "#1" is of sort "E" and "#2" of sort "A2", whereas in the second rule,
  "#1" is of sort "G", "#2" of sort "H", and here "#3" is of sort "A2". (Rules are very like "sugar"
...
\item Each rule should be equipped with a result (right of the~"→") of the appropriate sort, which
  can use the ``meta-variable'' "#"-named fragments using any (syntactic or semantic) mechanism we
  have for building something of the result sort. So if, for example, we have
\begin{hacs}
  sort R  |  ⟦ x ⟨E⟩ ⟧  |  scheme Other(A2);
\end{hacs}
  then we can write
\begin{hacs}
  sort R;
...
%%\begin{hacs}
%%  S(A1⟦ a ⟨E#1⟩ b ⟧, A2#2)        →   R⟦ x ⟨E#1⟩ ⟧ ;
%%  S(A1⟦ ⟨G#1⟩ c ⟨H#2⟩ ⟧, A2#3)    →   Other(A2#3) ;
%%\end{hacs}

\end{itemize}

\begin{example}[default rules]
  Since the last three cases of our "Leftmost" system in Example~\ref{ex:leftmost} above really just
...
\end{hacs}
  ...
  two occurrences of ``"_"'' should be filled with lists. Notice that the two rules differ on the
  \emph{content} of the braces, and are clearly designed to fully eliminate all possible contents of
  the braces: this is essential, and we say that the scheme should be \emph{complete}. Indeed it is
  \emph{syntax-directed in the first list}. 
\end{example}

Formally, syntactic schemes are just like semantic schemes but using syntax marked with ⟦⟧s.
However, syntactic schemes cannot be used in the parsed input, only the rules.

We can also define semantic data values.
...
\end{equation*}
%%
Rule \thetag{1} handles declarations in our toy language: it creates a \emph{new environment} (or
...
Finally, rule \thetag{3} uses the environment to attach another attribute $t$ passed into each
...
\item Next we encode the simplest rule, \thetag{2}. As in the previous section, we observe that
  \thetag{2} operates on sums, which in our case have a syntax case like this:
\begin{hacs}
   | ⟦ ⟨E⟩ + ⟨E@1⟩ ⟧
\end{hacs}
...
\item Let us do \thetag{3}. It defines the inherited attribute $t$ on \textbf{name} using a
...
\begin{hacs}
   Se(⟦⟨T#1⟩ name_2 = ⟨E#3⟩; ⟨S#4⟩⟧)  →      ⟦⟨T#1⟩ name_2 = ⟨E Ee(#3)⟩; ⟨S Se(#4)⟩⟧ ;
\end{hacs}
  ...
  map.
\begin{hacs}
   Se(⟦⟨T#1⟩ name_2 = ⟨E#3⟩; ⟨S#4⟩⟧)
   →   ⟦⟨T#1⟩ name_2 = ⟨E Ee(#3)⟩; ⟨S Se(#4) ↓e{⟦name_2⟧ : #1}⟩⟧ ;
\end{hacs}
