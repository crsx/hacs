...
\item[Precedence rules] are represented by the "@"-annotations, which assign precedence and
  ...
  left subexpressions of "*" of at least priority 1 (so "*" is also left recursive) whereas right
...
\item \emph{Precedence} is indicated with "@"$n$, where higher numbers $n$ designate higher
  ...
  non-terminal there is a precedence, which defaults to "@0".
...
\end{equation*}
%%
Rule \thetag{1} is a declaration that creates a modified version of the context symbol table
extended with an association of a name to its declared type. 

\begin{center}
  \textbf{To Be Completed.}
\end{center}

where we furthermore have that the $E.t$ property cannot be synthesized until \emph{after} the
inherited attribute $E.e$ has been ``spread'' into the term (we shall see later why this is
typical).
