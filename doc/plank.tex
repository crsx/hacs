...
\begin{definition}
  %%
  Given a sequence of forms $\ov{F}$ and an attribute name~$a$, we define the following
  notions:
  %%
  \begin{enumerate}

  \item $\op{carriers}(\ov{F}, a)$ is the set of all constructors $c$ for which there is a
    form in $\ov{F}$ of the shape ``$\kw{data}~c~…~a~…\kw;$'' or
    ``$\kw{scheme}~c~…~a~…\kw;$.''

  \item $\op{form}(\ov{D}, a)$ is the \set{AF} of the (single!) declaration
    ``$\kw{attribute}~a~\set{AF}\kw;$.''
