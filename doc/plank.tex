...
\begin{definition}[\hax syntax]
  ...
  \item $s,f,c ∈ \mathcal{C}$ stand for \emph{constructor} tokens, which are capitalized words like
    "Integer" or "A" or "CamelCaseWord", used for sort, function symbol, and data symbol names
    (respectively).
    \CK{But this suggests that when you write $c(\dots)$ that $c$ is a data symbol name.  You
      violate that right below when using it for arbitrary and function constructions.}
    \KR{Right. We could use $d$ for data and $c$ for ``unknown.''}

  \item $v,a,α ∈ \mathcal{V}$ stand for \emph{variable} tokens, which are lower case words like "x"
    or "foo" or even "lowWord", used for term variables, attributes, and sort variables
    (respectively).
  ...
  \item A term of the form $v\,A_1…A_m$ ($m≥0$) is a \emph{variable occurrence}; if the variable $v$
    occurs contained in a binder $[…v…]T$ then the variable occurrence is \emph{bound}.  \CK{Okay,
      ...
      applications, inclusing that substitution \emph{erases} those attributes.}
...
\LV{Lionel proposes this alternte syntax:}
\begin{code}
D  ::= sort S { F* } ;
...
\begin{example}[untyped λ calculus]\label{ex:lambda}
  ...
  \item The "Λ" sort includes a special "variable" case to indicate that variables can occur in
    ...
    \item in right-hand sides of rules, fresh variables of these sorts (and only these
      sorts) may occur.
    \end{itemize}
    Is all of that indeed the intention?  Also, the rephrasing I would propose depends on
    whether we separate terms (however you decide to call them) from meta-terms---which I
    think would be a really good thing to do, if only for clarity.}
...
\begin{example}[Boolean formulae]\label{ex:bool}
  ...
  \CK{I don't think this example works, though: in the end, you have all those formulas annotated,
    ...
    welcome.} \CK{Well, my suggestion would be to use a nullary constructor instead!}
\end{example}
...
\begin{definition}[sort environments]
  ...
  \item $Γ_{\op{cons-attr}}\colon \mathcal{C} → 2^{\mathcal{V}}$ from constructor name to set of
    attribute names. \\
    \CK{Should this be for \emph{function} constructors only?}
