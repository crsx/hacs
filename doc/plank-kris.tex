...
\begin{definition}[\hax syntax]
  ...
  \item $c,s,d,f ∈ \mathcal{C}$ stand for \emph{constructor} tokens, which are capitalized words like
    "Integer" or "A" or "CamelCaseWord". They are used for sort, function symbol, and data symbol
    names. By convention, we use $s$/$d$/$f$ for sort/data/function names, respectively, and $c$
    when either a data and function constructor makes sense.

    \CK{But this suggests that when you write $c(\dots)$ that $c$ is a data symbol name.  You
      violate that right below when using it for arbitrary and function constructions.}  \KR{OK?}
...
\begin{definition}
  ...
  \item $Γ(S)$ is defined as follows: $Γ(s[…]) = Γ(s)$ and $Γ(α\,\ov{a}) = \{\ov{a}\}$.
