...
syntax, hence also known as ``raw \HAX'').

\begin{definition}[\hax]
  The \hax grammar is shown in Figure~\ref{fig:rawhacs}. (We call this the ``raw'' grammar because it
  does not include the \HAX custom syntax notations.) The top level of a \HAX script is a sequence
  of declarations $D^*$. In addition, the grammar assumes that we have three categories of tokens
  defined:
  %%
  \begin{itemize}

  \item $s,f,c ∈ \mathbb{C}$ stands for \emph{constructor} tokens, which are capitalized words like
    "Integer" or "A" or "CamelCaseWord", used for sort names and function and data symbols.

  \item $a,v,α ∈ \mathbb{V}$ stands for \emph{variable} tokens, which are lower case words like "x" or
    "foo" or even "lowWord", used for variables and attributes.
...
\begin{example}[Peano]\label{ex:peano}
  ...
  \item All cases and rules are implicitly associated with the most recent \kw{sort}
    declaration.

  \item You can only define rules for function symbols declared with \kw{scheme}.
...
Let us define some really simple attribute cases.
