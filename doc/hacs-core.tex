...
\begin{definition}[\hax]
  ...
  \item $a,v,α ∈ \mathbb{V}$ stands for \emph{variable} tokens, which are lower case words like "x"
...
\begin{example}[Peano]\label{ex:peano}
  ...
  \item All cases and rules are implicitly associated with the wrapper \kw{sort} declaration.

  \item You can only define rules for function symbols declared with \kw{scheme}, \ie, the "Plus"
    symbol here.
