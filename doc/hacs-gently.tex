...
\title{
  ...
  \emph{Compiler Generation} \\
  Using \HAX\thanks{This version describes \HAX β version 0.9 released for use at NYU.}
}
\author{
  Kristoffer H. Rose\\
...
\begin{plan}
  ...
  Section~\ref{sec:sdd}, and higher-order abstract syntax in Section~\ref{sec:hoas}. In
  Section~\ref{sec:example} we walk through the full annotated \emph{First.hx} example.
...
$ ^textcolor{blue}{^texttt{make prefix=$HOME/.hacs install-support install}}
\end{code}
  %%
...
\begin{figure}[p]\small
\begin{code}[xleftmargin=1.66em,numbers=right]
...
\end{code}
  ...
  instructions.
\begin{code}[xleftmargin=1.66em,numbers=right,firstnumber=last]
| scheme Compile(Stat);
